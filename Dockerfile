...
#
# Build context is this repository, plus two named contexts for the unpublished packages:
#
#   docker build -t hub-api \
#     --build-context runtimepkgs=../runtime \
#     --build-context contractspkgs=../contracts .
#
# Both contexts are temporary. Once the @cloudsforge/* packages are published (AD-02), package.json
# takes registry versions, the COPY lines marked below are deleted, the flags go away, and this
# becomes an ordinary single-context build. Nothing else changes.
#
...
COPY --from=contractspkgs package.json pnpm-workspace.yaml /contracts/
...
# at runtime — this service has no database, no volume and no scratch space — so read-only
# ownership of the image is sufficient and the container needs no writable layer of its own.
USER node

# No secret is baked in, and none may be: every value in src/env.ts is supplied by the deploy at
# run time, and the image is published to a registry that is not the trust boundary for any of the
# six upstream credentials. There is no ENV line here on purpose.
...
# Invoked directly rather than through `pnpm start`, so the final image needs neither pnpm nor its
# dependency verification at boot. Once the packages are published this becomes
# ["node", "dist/index.js"] and tsx leaves the dependency tree entirely.
